...
export const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-[#071320] text-slate-300 pt-20 pb-10 border-t-4 border-[#D97745] relative overflow-hidden">
      {/* Subtle background branding watermark */}
      <div className="absolute -bottom-10 right-0 font-cinzel text-[10vw] font-black text-white/[0.02] select-none pointer-events-none whitespace-nowrap">
        CHINMAYA TARAPUR
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            >
              <div className="w-14 h-14 rounded-2xl bg-white p-1.5 flex items-center justify-center shrink-0 shadow-lg border border-white/20 group-hover:scale-105 transition-transform">
                <img src="/images/Chinmaya_Logo.webp" alt="Logo" className="w-full h-full object-contain" />
              </div>
              <div>
                <h3 className="font-cinzel font-bold text-white text-lg sm:text-xl tracking-tight leading-tight uppercase">
                  {OFFICIAL_SCHOOL_INFO.name}
                </h3>
                <p className="text-xs text-[#D97745] font-serif italic">"{OFFICIAL_SCHOOL_INFO.tagline}"</p>
              </div>
            </a>
            ...
            <div className="space-y-2 text-xs font-mono text-slate-300 pt-2 border-t border-white/10">
              <div className="flex items-center gap-2">
                <Award className="w-4 h-4 text-[#D97745]" />
                <span>CBSE Affiliation: <strong className="text-white">{OFFICIAL_SCHOOL_INFO.affiliationNo}</strong></span>
              </div>
              <div className="flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-[#D97745]" />
                <span>School Code: <strong className="text-white">{OFFICIAL_SCHOOL_INFO.schoolCode}</strong> | U-DISE: <strong className="text-white">{OFFICIAL_SCHOOL_INFO.udiseNo}</strong></span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            <h4 className="font-cinzel font-bold text-white text-sm uppercase tracking-wider border-b border-white/15 pb-2">
              Explore
            </h4>
            <ul className="space-y-2.5 text-xs font-sans">
              {[
                ...
                <li key={link.href}>
                  <Link to={link.href} className="flex items-center gap-1.5 text-slate-400 hover:text-[#D97745] transition-colors">
                    <ChevronRight className="w-3.5 h-3.5 text-[#D97745]/60" />
                ...
                <li key={idx}>
                  {item.external ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center gap-1.5 text-slate-400 hover:text-[#D97745] transition-colors"
                    >
                      <FileText className="w-3.5 h-3.5 text-[#D97745]/60" />
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <ul className="space-y-3.5 text-xs text-slate-400">
              ...
              <li className="flex items-center gap-2.5">
                ...
                <a href={`tel:${OFFICIAL_SCHOOL_INFO.contact.phone[0]}`} className="hover:text-white transition-colors">
                  {OFFICIAL_SCHOOL_INFO.contact.phone.join(" / ")}
                </a>
              </li>
              <li className="flex items-center gap-2.5">
                <Mail className="w-4 h-4 text-[#D97745] shrink-0" />
                <a href={`mailto:${OFFICIAL_SCHOOL_INFO.contact.email[0]}`} className="hover:text-white transition-colors truncate">
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs font-mono text-slate-400">
          ...
          <div className="flex items-center gap-6">
            ...
            <button
              onClick={scrollToTop}
              className="p-2 rounded-full bg-white/10 hover:bg-[#D97745] text-white transition-colors flex items-center gap-1 text-[11px] uppercase tracking-wider"
